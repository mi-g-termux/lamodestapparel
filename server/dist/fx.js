...
export function countryFromHeaders(headers) {
    ...
    return (pick("x-vercel-ip-country") ??
        pick("cf-ipcountry") ??
        pick("x-country-code") ??
        pick("x-geoip-country") ??
        pick("x-appengine-country"));
}
export function currencyForCountry(country, base) {
    if (!country)
        return base;
...
export async function getRate(base, quote) {
    if (base === quote)
        return 1;
    const row = await one(`select rate from exchange_rates where base = $1 and quote = $2`, [base, quote]);
    if (row?.rate)
        return Number(row.rate);
    // Try the inverse pair before giving up.
    const inv = await one(`select rate from exchange_rates where base = $1 and quote = $2`, [quote, base]);
    if (inv?.rate && Number(inv.rate) !== 0)
        return 1 / Number(inv.rate);
    return 1; // never break a page over a missing rate
}
export async function getAllRates(base) {
    const rows = await query(`select quote, rate from exchange_rates where base = $1`, [base]);
    const out = { [base]: 1 };
    for (const r of rows)
        out[r.quote] = Number(r.rate);
    return out;
}
/**
...
export async function refreshRates() {
    const cfg = await getNamespace("currency");
    ...
    const url = provider === "exchangerate_host"
    ...
    try {
        ...
        for (const [quote, rate] of Object.entries(rates)) {
            ...
            await query(`insert into exchange_rates (base, quote, rate, source, fetched_at)
         values ($1,$2,$3,$4, now())
         on conflict (base, quote)
         do update set rate = excluded.rate, source = excluded.source, fetched_at = now()`, [base, quote, adjusted, provider]);
            count++;
        }
        return { ok: true, count };
    }
    catch (err) {
        console.warn("[fx] refresh failed:", err.message);
...
export function convertMinor(minor, rate, fromDecimals, toDecimals, rounding = "none") {
...
export async function pricingContext(requested, country) {
    const cfg = await getNamespace("currency");
    const base = String(cfg.base ?? "GBP");
...
export function present(minor, ctx) {
