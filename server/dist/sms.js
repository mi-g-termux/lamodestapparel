 ...
 * strangers.
 */
import { TWILIO_API, VONAGE_API, MESSAGEBIRD_API, PLIVO_API, TELNYX_API, INFOBIP_PATH, WHATSAPP_API, withScheme, } from "./endpoints.js";
...
async function post(url, init, timeoutMs = 15_000) {
...
export async function sendSms(args) {
    ...
    if (!truthy(s.enabled))
        return { ok: false, provider: "disabled" };
    const provider = (text(s.provider) || "log");
    ...
    try {
        await query(`insert into sms_log (provider, recipient, body, status, error, message_id, order_id, kind)
...
async function dispatch(provider, s, to, body) {
    switch (provider) {
        case "twilio": {
            ...
            if (service)
                ...
                form.set("From", from);
            const r = await post(`${TWILIO_API}/Accounts/${encodeURIComponent(sid)}/Messages.json`, {
                method: "POST",
                headers: {
        ...
        case "plivo": {
            ...
            const r = await post(`${PLIVO_API}/${encodeURIComponent(id)}/Message/`, {
                method: "POST",
                headers: {
        ...
        case "whatsapp": {
            ...
            const r = await post(`${WHATSAPP_API}/${encodeURIComponent(phoneId)}/messages`, {
                method: "POST",
                headers: {
        ...
        case "generic": {
            ...
            const templateBody = text(s.generic_body_template) || '{"to":"{{to}}","text":"{{text}}"}';
            ...
            try {
                ...
                if (extra)
                    headers = { ...headers, ...JSON.parse(extra) };
            }
            catch {
                return {
                    ok: false,
...
export async function smsOrderPlaced(args) {
    const s = await smsSettings();
    if (!truthy(s.enabled) || !truthy(s.notify_order_placed) || !args.phone)
        return;
    const all = await getSettings();
    ...
    await sendSms({
        ...
        kind: "order_placed",
        body: text(s.template_order_placed)
            ?.split("{{order}}")
...
export async function smsOrderShipped(args) {
    const s = await smsSettings();
    if (!truthy(s.enabled) || !truthy(s.notify_order_shipped) || !args.phone)
        return;
    const all = await getSettings();
    const name = all?.branding?.store_name ?? "your store";
    await sendSms({
        ...
        orderId: args.orderId,
        kind: "order_shipped",
        body: text(s.template_order_shipped)
            ?.split("{{order}}")
            .join(args.orderNumber)
            .split("{{tracking}}")
            .join(args.tracking ?? "")
