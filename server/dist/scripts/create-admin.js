...
async function main() {
    ...
    if (!email.includes("@"))
        throw new Error("A valid email address is required (set ADMIN_EMAIL).");
    let generated = false;
    if (!password) {
    ...
    if (existing) {
        await query(`update admin_users set password_hash = $1, role = 'super_admin', status = 'active',
              failed_attempts = 0, locked_until = null, must_change_password = $2 where id = $3`, [hashPassword(password), generated, existing.id]);
        console.log(`\n✓ Reset the password for existing super admin ${email}`);
    }
    else {
        await query(`insert into admin_users (email, name, password_hash, role, status, must_change_password)
       values ($1,$2,$3,'super_admin','active',$4)`, [email.toLowerCase(), name || email, hashPassword(password), generated]);
