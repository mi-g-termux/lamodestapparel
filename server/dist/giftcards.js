...
function randomCode(groups = 4, size = 4) {
...
export const normaliseCode = (raw) => raw.toUpperCase().replace(/[^A-Z0-9]/g, "");
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
        ...
        try {
            const card = await tx(async (client) => {
                const { rows } = await client.query(`insert into gift_cards
                ...
                if (!created)
                    throw new Error("Gift card insert returned no row");
                await client.query(`insert into gift_card_transactions
 ...
 */
export async function checkBalance(code) {
    const secret = await codeSecret();
    const card = await one(`select id, balance_minor, currency, status, expires_at
		   from gift_cards where code_hash = $1`, [hashCode(code, secret)]);
    if (!card) {
        return {
            valid: false,
            balanceMinor: 0,
...
export async function redeem(args) {
    ...
    return tx(async (client) => {
        ...
        const { rows } = await client.query(`select id, balance_minor, currency, status, expires_at
			   from gift_cards where code_hash = $1 for update`, [hash]);
        const card = rows[0];
        if (!card)
            throw new Error("We could not find that gift card.");
        ...
        if (card.currency.toUpperCase() !== args.currency.toUpperCase()) {
            throw new Error(`This gift card is in ${card.currency} and cannot be used for a ${args.currency} order.`);
        }
        const applied = Math.min(card.balance_minor, Math.max(0, args.requestedMinor));
        if (applied <= 0)
            throw new Error("This gift card has no balance left.");
        const remaining = card.balance_minor - applied;
        await client.query(`update gift_cards
			    set balance_minor = $2,
			        status = case when $2 = 0 then 'redeemed' else status end,
			        updated_at = now()
			  where id = $1`, [card.id, remaining]);
        await client.query(`insert into gift_card_transactions
			   (gift_card_id, kind, amount_minor, balance_after_minor, order_id, actor)
			 values ($1,'redeem',$2,$3,$4,'storefront')`, [card.id, -applied, remaining, args.orderId]);
        await client.query(`insert into gift_card_redemptions (gift_card_id, order_id, amount_minor)
			 values ($1,$2,$3) on conflict do nothing`, [card.id, args.orderId, applied]);
        return {
...
export async function restoreForOrder(orderId, actor = "system") {
    const rows = await query(`select gift_card_id, amount_minor from gift_card_redemptions
		  where order_id = $1 and restored_at is null`, [orderId]);
    let restored = 0;
    for (const r of rows) {
        await tx(async (client) => {
            const { rows: cards } = await client.query(`select balance_minor from gift_cards where id = $1 for update`, [r.gift_card_id]);
            if (!cards[0])
                return;
            const balance = cards[0].balance_minor + r.amount_minor;
            await client.query(`update gift_cards set balance_minor = $2, status = 'active', updated_at = now()
				  where id = $1`, [r.gift_card_id, balance]);
            await client.query(`insert into gift_card_transactions
				   (gift_card_id, kind, amount_minor, balance_after_minor, order_id, actor, note)
				 values ($1,'restore',$2,$3,$4,$5,'Order cancelled or refunded')`, [r.gift_card_id, r.amount_minor, balance, orderId, actor]);
            await client.query(`update gift_card_redemptions set restored_at = now()
				  where gift_card_id = $1 and order_id = $2`, [r.gift_card_id, orderId]);
...
export async function adjustBalance(args) {
    return tx(async (client) => {
        const { rows } = await client.query(`select balance_minor from gift_cards where id = $1 for update`, [args.giftCardId]);
        if (!rows[0])
        ...
        await client.query(`update gift_cards
			    set balance_minor = $2,
			        ...
			        updated_at = now()
			  where id = $1`, [args.giftCardId, balance]);
        await client.query(`insert into gift_card_transactions
			   (gift_card_id, kind, amount_minor, balance_after_minor, actor, note)
			 values ($1,'adjust',$2,$3,$4,$5)`, [args.giftCardId, args.deltaMinor, balance, args.actor, args.note ?? null]);
        return balance;
    });
}
export async function setStatus(giftCardId, status, actor) {
    await query(`update gift_cards set status = $2, updated_at = now() where id = $1`, [giftCardId, status]);
    await query(`insert into gift_card_transactions
		   (gift_card_id, kind, amount_minor, balance_after_minor, actor, note)
		 select $1, $2, 0, balance_minor, $3, null from gift_cards where id = $1`, [giftCardId, status === "disabled" ? "disable" : "enable", actor]);
}
/** Admin list, newest first. Never exposes a spendable code. */
export async function listGiftCards(args) {
    const limit = Math.min(200, Math.max(1, args.limit ?? 50));
    const rows = await query(`select id, code_last4, initial_minor, balance_minor, currency, status,
		        expires_at, recipient_email, note, created_at
		   from gift_cards
		  where ($3::text is null or status = $3)
		  order by created_at desc
		  limit $1 offset $2`, [limit, Math.max(0, args.offset ?? 0), args.status ?? null]);
    return rows;
}
export async function giftCardHistory(giftCardId) {
    const rows = await query(`select kind, amount_minor, balance_after_minor, order_id, actor, note, created_at
		   from gift_card_transactions
		  where gift_card_id = $1 order by created_at asc`, [giftCardId]);
    return rows;
}
/** Outstanding liability — money you owe in unspent cards. */
export async function outstandingLiability() {
    const row = await one(`select coalesce(sum(balance_minor),0) as total, count(*) as count,
