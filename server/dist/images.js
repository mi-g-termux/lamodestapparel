...
const FORMATS = { avif: "image/avif", webp: "image/webp", jpeg: "image/jpeg" };
...
async function loadSharp() {
    ...
    try {
        ...
        console.warn("[images] sharp is not installed — originals will be served as-is. " +
...
function safeSource(name) {
    const clean = path.basename(name.replace(/\\/g, "/"));
    if (!clean || clean.startsWith("."))
        return null;
    if (!/\.(jpe?g|png|webp|avif|gif|tiff?)$/i.test(clean))
        return null;
...
function pickFormat(accept, requested) {
    if (requested && requested in FORMATS)
        return requested;
    const a = accept ?? "";
    if (a.includes("image/avif"))
        return "avif";
    if (a.includes("image/webp"))
        return "webp";
    return "jpeg";
}
async function transcode(source, width, format, quality) {
 ...
 */
export async function makeLqip(source) {
    const sharp = await loadSharp();
    if (!sharp)
        return null;
    try {
...
export async function imageMeta(source) {
    const sharp = await loadSharp();
    if (!sharp)
        return {};
    try {
 ...
 */
export async function normaliseUpload(source, maxWidth = 2560) {
    const sharp = await loadSharp();
    if (!sharp)
        return;
    try {
...
imagesRouter.get("/:file", async (req, res) => {
    ...
    if (!source)
        return res.status(400).json({ error: "Bad image request." });
    try {
    ...
    const width = ALLOWED_WIDTHS.find((w) => w >= requestedWidth) ??
        ALLOWED_WIDTHS[ALLOWED_WIDTHS.length - 1] ??
        1024;
    const quality = Math.min(95, Math.max(40, Number(req.query.q ?? 75) || 75));
...
export function srcset(file, formats = "webp") {
    return ALLOWED_WIDTHS.map((w) => `/api/img/${encodeURIComponent(file)}?w=${w}&f=${formats} ${w}w`).join(", ");
}
/** Drop cached variants when a file is replaced or deleted. */
export async function clearImageCache() {
    try {
        const files = await fs.readdir(cacheDir());
        await Promise.all(files.map((f) => fs.unlink(path.join(cacheDir(), f)).catch(() => { })));
        return files.length;
    }
    catch {
        return 0;
