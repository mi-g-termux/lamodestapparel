...
import { getPublicSettings, featureOn } from "../settings.js";
import { pricingContext, present, countryFromHeaders, CURRENCY_META, getAllRates, } from "../fx.js";
...
async function ctxFor(req) {
    const requested = String(req.query.currency ?? req.headers["x-currency"] ?? "") || null;
    const country = (String(req.query.country ?? "") || null) ??
        countryFromHeaders(req.headers);
    return pricingContext(requested, country ?? countryFromHeaders(req.headers));
}
...
publicRouter.get("/bootstrap", async (req, res) => {
    ...
    res.json({
        ...
        pricing: {
            ...
            detectedCountry: countryFromHeaders(req.headers),
...
publicRouter.get("/banners", async (req, res) => {
    ...
    const rows = await query(`select b.id, b.placement, b.title, b.subtitle, b.body, b.cta_label, b.cta_href,
      ...
      order by b.placement, b.position`, [placement]);
    res.json({ banners: rows });
});
publicRouter.get("/pages/:slug", async (req, res) => {
    const page = await one(`select slug, title, body, seo_title, seo_description, updated_at
       from pages where slug = $1 and published`, [req.params.slug]);
    if (!page) {
...
publicRouter.get("/testimonials", async (_req, res) => {
    ...
    res.json({
        testimonials: await query(`select id, author, role, body, rating, avatar_url from testimonials where active order by position`),
    });
});
/* ---------------- catalogue --------------------------------------------- */
async function decorateProducts(rows, ctx) {
    const ids = rows.map((r) => String(r.id));
    if (!ids.length)
        return [];
    const images = await query(`select pi.product_id, m.url, m.alt, pi.position
       from product_images pi join media m on m.id = pi.media_id
      where pi.product_id = any($1::uuid[]) order by pi.position`, [ids]);
    const cats = await query(`select pc.product_id, c.slug, c.name from product_categories pc
...
publicRouter.get("/products", async (req, res) => {
    ...
    const order = sort === "price_asc" ? "p.price_minor asc" :
        sort === "price_desc" ? "p.price_minor desc" :
            sort === "title" ? "p.title asc" : "p.created_at desc";
    const rows = await query(`select distinct p.* from products p
      ...
      order by ${order} limit $4 offset $5`, [search, category, featured, limit, offset]);
    const total = await one(`select count(distinct p.id)::int as n from products p
       left join product_categories pc on pc.product_id = p.id
       left join categories c on c.id = pc.category_id
      where p.status = 'active'
        and ($1 = '' or p.title ilike '%' || $1 || '%' or p.description ilike '%' || $1 || '%')
        and ($2 = '' or c.slug = $2)
        and ($3 = false or p.featured)`, [search, category, featured]);
...
publicRouter.get("/products/:slug", async (req, res) => {
    ...
    const variants = await query(`select id, title, options, sku, price_minor, stock from product_variants where product_id = $1 order by position`, [String(row.id)]);
    const reviews = (await featureOn("reviews"))
        ? await query(`select author, rating, title, body, created_at from product_reviews
          where product_id = $1 and state = 'published' order by created_at desc limit 30`, [String(row.id)])
        : [];
    res.json({
        product: {
            ...product,
...
publicRouter.get("/categories", async (_req, res) => {
    res.json({
        categories: await query(`select c.id, c.slug, c.name, c.description, m.url as image_url,
...
publicRouter.get("/currencies", async (req, res) => {
    ...
    res.json({
        base: ctx.base,
        display: ctx.display,
        detected: countryFromHeaders(req.headers),
...
/* ---------------- checkout ---------------------------------------------- */
publicRouter.post("/checkout", rateLimit("checkout", 12, 600), async (req, res) => {
    const body = req.body;
    const ctx = await ctxFor(req);
...
async function findOrder(numberOrId, email) {
    return one(`select * from orders where (number = $1 or id::text = $1) and lower(email) = lower($2)`, [numberOrId, email]);
}
publicRouter.get("/orders/lookup", rateLimit("order_lookup", 30, 600), async (req, res) => {
    ...
    const history = await query(`select field, to_value, note, created_at from order_status_history where order_id = $1 and field = 'status' order by created_at`, [order.id]);
    res.json({ order, items, history });
});
publicRouter.get("/orders/:id/invoice.pdf", rateLimit("invoice", 30, 600), async (req, res) => {
...
/* ---------------- small forms ------------------------------------------- */
publicRouter.post("/newsletter", rateLimit("newsletter", 6, 600), async (req, res) => {
    ...
    await query(`insert into newsletter_subscribers (email, source) values ($1,'storefront')
     on conflict (email) do update set status = 'subscribed'`, [email]);
    res.json({ ok: true });
});
publicRouter.post("/contact", rateLimit("contact", 6, 600), async (req, res) => {
...
publicRouter.post("/reviews", rateLimit("review", 5, 3600), async (req, res) => {
    ...
    await query(`insert into product_reviews (product_id, author, email, rating, title, body, state)
...
publicRouter.post("/back-in-stock", rateLimit("bis", 10, 3600), async (req, res) => {
...
publicRouter.post("/coupons/validate", rateLimit("coupon", 20, 600), async (req, res) => {
    const code = String(req.body?.code ?? "").trim();
    const subtotal = Number(req.body?.subtotalMinor ?? 0);
    const c = await one(`select * from coupons where lower(code) = lower($1)`, [code]);
    const now = Date.now();
    const ok = !!c && c.active &&
