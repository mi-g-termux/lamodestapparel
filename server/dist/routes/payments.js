...
import { availableMethods, methodIsAllowed, offlineInstructions, stripeConfig, paypalConfig, stripe, paypal, PaymentError, } from "../payments/index.js";
export const paymentsRouter = Router();
const siteUrl = () => env.SITE_URL.replace(/\/+$/, "");
async function loadOrder(id) {
    return one(`select id, number as order_number, email, currency, presentment_currency,
...
async function orderLines(orderId) {
    ...
    return await query(`select title, sku, qty as quantity, unit_price_minor
		   from order_items where order_id = $1 order by id asc`, [orderId]);
}
async function storeName() {
    const s = await getSettings();
    return (s?.branding?.store_name ??
...
paymentsRouter.post("/start", async (req, res) => {
    ...
    try {
        if (method === "stripe") {
            ...
            await query(`update orders set payment_method = $2, payment_reference = $3,
				        payment_status = case when payment_status = 'unpaid' then 'pending' else payment_status end,
				        updated_at = now()
				  where id = $1`, [order.id, "stripe", session.id]);
            await query(`insert into payments (order_id, provider, provider_reference, amount_minor, currency, status)
				 values ($1,$2,$3,$4,$5,'pending')
				 on conflict do nothing`, [order.id, "stripe", session.id, order.total_minor, currency]);
            return res.json({
                flow: "redirect",
                provider: "stripe",
                url: session.url,
        ...
        if (method === "paypal") {
            ...
            await query(`update orders set payment_method = $2, payment_reference = $3,
				        payment_status = case when payment_status = 'unpaid' then 'pending' else payment_status end,
				        updated_at = now()
				  where id = $1`, [order.id, "paypal", created.id]);
            await query(`insert into payments (order_id, provider, provider_reference, amount_minor, currency, status)
				 values ($1,$2,$3,$4,$5,'pending')
				 on conflict do nothing`, [order.id, "paypal", created.id, order.total_minor, currency]);
            return res.json({
                flow: "redirect",
                provider: "paypal",
        ...
        await query(`update orders set payment_method = $2, payment_status = 'unpaid', updated_at = now()
			  where id = $1`, [order.id, method]);
        return res.json({
            flow: "offline",
            provider: method,
        ...
        return res.status(502).json({
            error: err instanceof PaymentError || err?.name?.includes("Error")
                ? err.message
...
async function markPaid(args) {
    return tx(async (client) => {
        ...
        const { rows } = await client.query(`select id, number as order_number, email, currency, presentment_currency,
			        total_minor, subtotal_minor, shipping_minor, tax_minor, discount_minor,
			        status, payment_status, payment_method, payment_reference, capture_reference
			   from orders where id = $1 for update`, [args.orderId]);
        const order = rows[0];
        if (!order)
        ...
        if (mismatch) {
            await client.query(`update orders set payment_status = 'partially_paid', status = 'on_hold',
				        payment_reference = $2, updated_at = now()
				  where id = $1`, [order.id, args.reference]);
            await client.query(`insert into order_status_history (order_id, field, from_value, to_value, note, actor)
        ...
        await client.query(`update orders
			    set payment_status = 'paid',
			        ...
			        paid_at = now(),
			        updated_at = now()
			  where id = $1`, [order.id, args.provider, args.reference, args.captureReference ?? null]);
        await client.query(`insert into payments (order_id, provider, provider_reference, amount_minor, currency, status, captured_at)
...
async function afterPaid(orderId, orderNumber) {
    ...
    try {
        ...
        const order = await one(`select id, number, email, currency, total_minor, fx_rate, payment_method,
			        tracking_number, tracking_url, shipping_address
			   from orders where id = $1`, [orderId]);
        if (order) {
            const items = await query(`select title, variant_title, qty, total_minor
				   from order_items where order_id = $1 order by id asc`, [orderId]);
...
paymentsRouter.get("/return", async (req, res) => {
    ...
    try {
        if (provider === "stripe") {
            ...
            if (!cfg || !sessionId)
                throw new PaymentError("Missing Stripe session.");
            ...
            if (!paid) {
                return res.json({
                    status: "pending",
                    orderNumber: order.order_number,
                    message: "Your payment is still being confirmed. We will email you the moment it clears.",
                });
            }
            ...
            const result = await markPaid({
                ...
                paidMinor: stripe.fromStripeAmount(Number(session.amount_total ?? 0), currency),
                currency,
            });
            if (result.mismatch) {
                return res.json({
                    status: "review",
                    orderNumber: order.order_number,
                    message: "We received a payment for a different amount, so your order is being checked by our team.",
                });
            }
            if (result.changed)
                await afterPaid(order.id, order.order_number);
            return res.json({ status: "paid", orderNumber: order.order_number });
        }
        if (provider === "paypal") {
            const cfg = await paypalConfig();
            const paypalOrderId = String(req.query.token ?? "") || order.payment_reference || "";
            if (!cfg || !paypalOrderId)
                throw new PaymentError("Missing PayPal order.");
            const captured = await paypal.captureOrder(cfg, paypalOrderId);
            if (!/COMPLETED/i.test(captured.status)) {
                return res.json({
                    status: "pending",
                    orderNumber: order.order_number,
                    message: "PayPal has not finished confirming this payment. We will email you when it clears.",
                });
            }
            const result = await markPaid({
                orderId: order.id,
        ...
        return res.status(502).json({
            status: "unknown",
            orderNumber: order.order_number,
            error: "We could not confirm the payment just now. If money left your account, your order is safe and our team will see it.",
        });
    }
});
/** Shopper backed out. Leave the order alone so they can try again. */
paymentsRouter.get("/cancelled", async (req, res) => {
    const orderId = String(req.query.order ?? "");
    if (!orderId || !verifyOrderToken(orderId, req.query.t)) {
        return res.status(400).json({ error: "This payment link is not valid." });
    }
...
async function alreadySeen(provider, eventId) {
    const rows = await query(`insert into webhook_events (provider, event_id, received_at)
		 values ($1,$2, now())
		 on conflict (provider, event_id) do nothing
		 returning event_id`, [provider, eventId]);
...
paymentsRouter.post("/webhook/stripe", async (req, res) => {
    ...
    const verified = stripe.verifyWebhook(Buffer.isBuffer(raw) ? raw : String(raw), req.header("stripe-signature"), cfg.webhookSecret);
    if (!verified.ok) {
    ...
    try {
        if (await alreadySeen("stripe", event.id))
        ...
        if (event.type === "checkout.session.completed" ||
            ...
            if (orderId) {
                ...
                const reference = typeof obj.payment_intent === "string"
                    ? obj.payment_intent
                    : obj.id;
                const result = await markPaid({
                    orderId,
            ...
            if (orderId) {
                await query(`update orders set payment_status = 'failed', updated_at = now()
            ...
            if (intent) {
                ...
                await query(`update orders
					  ...
					  where payment_reference = $1`, [intent, fully ? "refunded" : "partially_refunded"]);
            }
        }
        else if (event.type === "charge.dispute.created") {
            const intent = typeof obj.payment_intent === "string" ? obj.payment_intent : null;
            if (intent) {
                await query(`update orders set status = 'on_hold', updated_at = now()
					  where payment_reference = $1`, [intent]);
            }
        }
    }
    catch (err) {
...
paymentsRouter.post("/webhook/paypal", async (req, res) => {
    const cfg = await paypalConfig();
    if (!cfg?.webhookId) {
    ...
    const verified = await paypal.verifyWebhook(cfg, req.headers, body);
    if (!verified.ok) {
    ...
    try {
        ...
        if (event.event_type === "PAYMENT.CAPTURE.COMPLETED" && orderId) {
            ...
            const result = await markPaid({
                orderId,
                provider: "paypal",
                reference: resource.supplementary_data?.related_ids?.order_id ?? resource.id,
        ...
        else if (["PAYMENT.CAPTURE.REFUNDED", "PAYMENT.CAPTURE.REVERSED"].includes(event.event_type)) {
            const captureId = resource.links
                ?.find((l) => l.rel === "up")
                ?.href?.split("/")
                .pop() ?? null;
            if (captureId) {
                await query(`update orders set payment_status = 'refunded', updated_at = now()
					  where capture_reference = $1`, [captureId]);
            }
        }
        else if (event.event_type === "CUSTOMER.DISPUTE.CREATED" && orderId) {
            await query(`update orders set status = 'on_hold', updated_at = now() where id = $1`, [orderId]);
        }
    }
    catch (err) {
