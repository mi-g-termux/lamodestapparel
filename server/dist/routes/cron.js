...
function authorised(req) {
    ...
    if (supplied.length !== env.CRON_SECRET.length)
        return false;
    try {
...
cronRouter.all("/low-stock", async (_req, res) => {
    const s = await getSettings();
    if (s.notifications?.on_low_stock === false) {
        res.json({ skipped: true });
        return;
    }
    const rows = await query(`select id, title, stock from products
...
cronRouter.all("/stuck-payments", async (_req, res) => {
    const rows = await query(`update orders set status = 'failed', updated_at = now()
