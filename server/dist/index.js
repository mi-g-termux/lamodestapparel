...
app.use("/api/public", async (req, res, next) => {
    if (await isMaintenanceMode()) {
        ...
        res.status(503).json({
            maintenance: true,
            message: settings.features?.maintenance_message ??
...
app.get("*", async (_req, res) => {
    try {
        ...
        res.status(500).send("<h1>Front end not built</h1><p>Run <code>npm run build</code> in the <code>web</code> folder.</p>");
    }
});
/* ---------------- error handling ---------------- */
