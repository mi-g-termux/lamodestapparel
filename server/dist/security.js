...
export function passwordProblems(pw) {
    const out = [];
    if (pw.length < 12)
        out.push("must be at least 12 characters");
    if (!/[a-z]/.test(pw))
        out.push("needs a lowercase letter");
    if (!/[A-Z]/.test(pw))
        out.push("needs an uppercase letter");
    if (!/[0-9]/.test(pw))
        out.push("needs a number");
    if (/^(password|velora|admin|12345)/i.test(pw))
        out.push("is too predictable");
    return out;
}
/* ------------------------------------------------------------------ *
...
export async function createSession(userId, req, res) {
    ...
    await query(`insert into admin_sessions (user_id, token_hash, ip, user_agent, expires_at)
     values ($1,$2,$3,$4, now() + interval '${SESSION_DAYS} days')`, [userId, sha256(token), clientIp(req), String(req.headers["user-agent"] ?? "").slice(0, 300)]);
...
export async function userFromRequest(req) {
    ...
    if (!token)
        return null;
    const row = await one(`select u.id, u.email, u.name, u.role, u.overrides, u.status, u.avatar_url,
      ...
      where s.token_hash = $1
        ...
        and u.status = 'active'`, [sha256(token)]);
    return row;
}
/* ------------------------------------------------------------------ *
...
export function lockoutMinutesFor(failures) {
    if (failures < 5)
        return 0;
...
export function rateLimit(bucket, limit, windowSeconds) {
    return async (req, res, next) => {
        ...
        try {
            ...
            const rows = await query(`select count(*)::int as n from rate_limits
          where bucket = $1 and ip = $2 and created_at > now() - ($3 || ' seconds')::interval`, [bucket, ip, String(windowSeconds)]);
            if ((rows[0]?.n ?? 0) >= limit) {
...
const ROLE_PERMS = {
    super_admin: "*",
    developer: "*",
    admin: ALL_PERMISSIONS.filter((p) => p !== "system.manage" && p !== "user.manage"),
    manager: [
...
export function permissionsFor(role, overrides = []) {
    ...
    const set = new Set(base === "*" ? ALL_PERMISSIONS : (base ?? []));
    for (const o of overrides) {
        if (o.startsWith("-"))
            set.delete(o.slice(1));
        else
            set.add(o);
    }
    return set;
...
export function isDeveloperZone(user) {
