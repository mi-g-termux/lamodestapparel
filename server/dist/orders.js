...
function formatOrderNumber(seq, s) {
...
async function nextOrderNumber() {
    const s = await getNamespace("orders");
    const row = await one(`select nextval('order_number_seq') as n`);
...
export async function createOrder(args) {
    ...
    for (const item of wanted) {
        const product = await one(`select id, title, status, price_minor, sku, track_stock, stock, tax_class
        ...
        if (item.variantId) {
            const variant = await one(`select id, title, sku, price_minor, stock
				   from product_variants where id = $1 and product_id = $2`, [item.variantId, product.id]);
            if (!variant)
        ...
        const image = await one(`select url from product_images where product_id = $1 order by position asc limit 1`, [product.id]);
        lines.push({
    ...
    if (rawCoupon) {
        const coupon = await one(`select id, code, type, value, min_spend_minor, usage_limit, used_count,
			        active, starts_at, ends_at
			   from coupons where upper(code) = upper($1)`, [rawCoupon]);
        const now = Date.now();
        ...
        const usageOk = !coupon?.usage_limit || Number(coupon.used_count ?? 0) < Number(coupon.usage_limit);
    ...
    if (taxSettings.enabled === true) {
        const percent = Number(taxSettings.rate_percent ??
            taxSettings.default_rate ??
            taxSettings.rate ??
            0);
    ...
    const totalMinor = taxSettings.prices_include_tax === true
        ? subtotalMinor - discountMinor + shippingMinor
        : subtotalMinor - discountMinor + shippingMinor + taxMinor;
    if (totalMinor < 0)
        return { ok: false, error: "We could not total that basket." };
    ...
    try {
        const orderId = await tx(async (client) => {
            ...
            if (!customerId) {
                const created = await client.query(`insert into customers (email, name, phone)
					 values ($1, $2, $3)
					 on conflict (email) do update set phone = coalesce(customers.phone, excluded.phone)
					 returning id`, [
                    email,
                    String(args.shippingAddress?.name ??
                        args.billingAddress?.name ??
                        ""),
            ...
            for (const l of lines) {
                ...
                if (l.trackStock) {
                    if (l.variantId) {
                        const r = await client.query(`update product_variants set stock = stock - $2
							  where id = $1 and stock >= $2`, [l.variantId, l.qty]);
                        if (r.rowCount === 0)
                            throw new Error(`SOLD_OUT:${l.title}`);
                    }
                    const r = await client.query(`update products set stock = stock - $2, updated_at = now()
						  where id = $1 and stock >= $2`, [l.productId, l.qty]);
                    if (r.rowCount === 0)
                        throw new Error(`SOLD_OUT:${l.title}`);
                }
            }
            if (couponCode) {
                ...
                await client
                    .query(`insert into coupon_redemptions (coupon_code, order_id, email)
						 values ($1,$2,$3)`, [couponCode, newId, email])
                    .catch(() => {
            ...
            await client.query(`insert into order_status_history (order_id, field, from_value, to_value, note, actor)
				 values ($1,'status',null,'pending',$2,'customer')`, [newId, `Placed via ${paymentMethod}`]);
            // Offline methods have nothing to confirm, so the order can move on.
            if (autoConfirm && (paymentMethod === "cod" || paymentMethod === "manual")) {
                await client.query(`update orders set status = 'confirmed', updated_at = now() where id = $1`, [newId]);
            }
            return newId;
        });
 ...
 */
export async function updateOrderField(orderId, field, value, actor, note) {
    ...
    const current = await one(`select id, status, payment_status, fulfillment_status, tracking_number,
    ...
    await query(`insert into order_status_history (order_id, field, from_value, to_value, note, actor)
		 values ($1,$2,$3,$4,$5,$6)`, [orderId, field, before, after, note ?? null, actor]);
    /* Keep the obvious companions in step so staff cannot leave a contradiction. */
    if (field === "status" && (value === "delivered" || value === "completed")) {
        await query(`update orders set fulfillment_status = 'fulfilled', updated_at = now()
			  where id = $1 and fulfillment_status <> 'returned'`, [orderId]);
    }
    if (field === "status" && value === "shipped") {
        await query(`update orders set fulfillment_status = 'fulfilled', updated_at = now()
			  where id = $1 and fulfillment_status = 'unfulfilled'`, [orderId]);
    }
    return { ok: true };
}
/* ------------------------------------------------------------------ *
 ...
 */
export async function revenueSeries(granularity = "day", days = 30) {
    ...
    const windowDays = Number.isFinite(windowDaysRaw)
        ...
        : 30;
    const rows = await query(`with span as (
		 ...
		 select to_char(span.bucket, 'YYYY-MM-DD')                     as bucket,
		  ...
		  order by span.bucket`, [unit, windowDays]);
    return rows.map((r) => ({
...
export async function topProducts(limit = 10) {
    ...
    const rows = await query(`select oi.product_id,
