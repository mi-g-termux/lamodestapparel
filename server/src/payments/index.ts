...
export type StartResult =
	| { flow: "redirect"; provider: ProviderId; url: string; reference: string }
	| { flow: "offline"; provider: ProviderId; instructions: string }
...
type PaymentsSettings = Record<string, any>
 ...
 */
async function paymentSettings(): Promise<PaymentsSettings> {
	const all: any = await getSettings()
	const p: PaymentsSettings = { ...(all?.payments ?? {}) }
...
export async function stripeConfig(): Promise<stripe.StripeConfig | null> {
	const p = await paymentSettings()
	if (!truthy(p.stripe_enabled)) return null
...
export async function paypalConfig(): Promise<paypal.PayPalConfig | null> {
	const p = await paymentSettings()
	if (!truthy(p.paypal_enabled)) return null
 ...
 */
export async function availableMethods(): Promise<
	Array<{
		id: ProviderId
		label: string
		description: string
...
> {
	...
	if (paypalCfg) {
		out.push({
			...
			description:
				text(p.paypal_description) || "Pay with your PayPal balance or card.",
			flow: "redirect",
			ready: true,
		})
	}
	...
	if (truthy(p.cod_enabled)) {
		out.push({
			...
			description:
				text(p.cod_instructions) || "Pay the courier when your order arrives.",
			flow: "offline",
			ready: true,
		})
	}
...
export async function methodIsAllowed(id: string): Promise<boolean> {
...
export async function offlineInstructions(id: ProviderId): Promise<string> {
	const p = await paymentSettings()
	if (id === "bank_transfer") {
		const lines = [
			text(p.bank_instructions),
...
export async function refundViaProvider(args: {
	...
	switch (args.provider) {
		case "stripe": {
			...
			if (!cfg) throw new PaymentError("Stripe is not configured.", "stripe")
			if (!args.providerReference) {
				throw new PaymentError(
					"This order has no Stripe payment reference, so it cannot be refunded automatically.",
					"stripe",
				)
			}
		...
		case "paypal": {
			...
			if (!cfg) throw new PaymentError("PayPal is not configured.", "paypal")
			const captureId = args.captureReference ?? args.providerReference
			if (!captureId) {
				throw new PaymentError(
					"This order has no PayPal capture reference, so it cannot be refunded automatically.",
					"paypal",
				)
			}
