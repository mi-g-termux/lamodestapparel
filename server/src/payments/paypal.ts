...
export function toDecimal(minor: number, currency: string): string {
	...
	if (ZERO_DECIMAL.has(c)) return String(Math.round(minor / 100))
	return (Math.round(minor) / 100).toFixed(2)
}

export function fromDecimal(value: string, currency: string): number {
...
async function accessToken(cfg: PayPalConfig): Promise<string> {
	if (!cfg.clientId || !cfg.clientSecret) {
		throw new PayPalError(
			"PayPal is switched on but the client ID and secret have not been saved in Settings → Payments.",
		)
	}
	...
	if (!res.ok || !json.access_token) {
		...
		throw new PayPalError(
			json.error_description ??
				`PayPal rejected the credentials (${res.status}). Check the client ID and secret, and that they match the ${cfg.mode} environment.`,
			json.debug_id,
			res.status,
		)
	}
...
async function call<T = any>(
	...
	const json: any = text ? safeJson(text) : {}
...
export type PayPalOrderArgs = {
	orderId: string
	orderNumber: string
	currency: string
	lines: Array<{ name: string; amountMinor: number; quantity: number; sku?: string }>
...
export async function createOrder(
		...
		purchase_units: [
			{
				...
				description: `${args.storeName} order ${args.orderNumber}`.slice(
					0,
					127,
				),
				amount: {
	...
	const order = await call<any>(cfg, "POST", "/v2/checkout/orders", body, {
	...
	if (!approve?.href) {
		throw new PayPalError(
			"PayPal created the order but returned no approval link.",
		)
	}
	return { id: order.id, approveUrl: approve.href }
}

export async function getOrder(cfg: PayPalConfig, id: string) {
	return call<any>(cfg, "GET", `/v2/checkout/orders/${encodeURIComponent(id)}`)
}
...
export async function refundCapture(
	...
	if (args.note) body.note_to_payer = args.note.slice(0, 255)

	const r = await call<any>(
		cfg,
		"POST",
		`/v2/payments/captures/${encodeURIComponent(args.captureId)}/refund`,
		body,
...
export async function verifyWebhook(
	...
	rawBody: string,
): Promise<{ ok: boolean; reason?: string; event?: any }> {
	if (!cfg.webhookId) {
...
export function eventFingerprint(rawBody: string): string {
