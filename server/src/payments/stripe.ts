...
export type StripeConfig = {
	...
	/** Stripe wants amounts in minor units, except for zero-decimal currencies. */
...
async function call<T = any>(
	...
	if (!cfg.secretKey) {
		throw new StripeError(
			"Stripe is switched on but no secret key has been saved in Settings → Payments.",
			"config",
		)
	}
	...
	try {
		...
		throw new StripeError(
			err?.name === "AbortError"
				...
				: `Could not reach Stripe: ${err?.message ?? "network error"}`,
			"network",
		)
	}
	clearTimeout(timer)
	...
	if (!res.ok) {
		...
		throw new StripeError(
			e.message ?? `Stripe returned ${res.status}`,
			e.type ?? "api_error",
			res.status,
		)
	}
...
export async function verifyKey(cfg: StripeConfig): Promise<{
	ok: boolean
	mode: "live" | "test"
	account?: string
	error?: string
	...
	try {
		const acct = await call<any>(cfg, "GET", "/balance")
...
export type CheckoutArgs = {
	...
	orderNumber: string
	currency: string
	/** Line items already priced by our server, in minor units. */
	lines: Array<{ name: string; amountMinor: number; quantity: number }>
...
export async function createCheckoutSession(
	...
	if (cfg.statementDescriptor) {
		;(body.payment_intent_data as any).statement_descriptor_suffix =
	...
	const session = await call<any>(cfg, "POST", "/checkout/sessions", body, {
...
export async function retrieveSession(cfg: StripeConfig, id: string) {
	return call<any>(cfg, "GET", `/checkout/sessions/${encodeURIComponent(id)}`, {
		expand: ["payment_intent"],
	})
}

export async function retrievePaymentIntent(cfg: StripeConfig, id: string) {
	return call<any>(cfg, "GET", `/payment_intents/${encodeURIComponent(id)}`)
}
...
export async function refund(
	...
	const r = await call<any>(cfg, "POST", "/refunds", body, {
...
export function verifyWebhook(
	...
	if (!webhookSecret) return { ok: false, reason: "No webhook secret saved" }
