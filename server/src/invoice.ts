...
/**
 * Invoices.
 *  - a numbered invoice row is created the first time one is requested
 *  - the PDF is generated on the fly (works on Vercel's read-only filesystem)
 *  - the same generator powers the admin "Download invoice" button and the
 *    customer's own copy on the order page
 */
...
export async function ensureInvoice(orderId: string): Promise<{ number: string; issued_at: string }> {
  const existing = await one<{ number: string; issued_at: string }>(
    `select number, issued_at from invoices where order_id = $1`,
    [orderId],
  );
  ...
  const seq = await one<{ nextval: number }>(`select nextval('invoice_number_seq') as nextval`);
  const number = `${prefix}${String(seq?.nextval ?? 1).padStart(padding, "0")}`;

  const order = await one<{ total_minor: number; currency: string }>(
    `select total_minor, currency from orders where id = $1`,
    [orderId],
  );

  const row = await one<{ number: string; issued_at: string }>(
    `insert into invoices (order_id, number, total_minor, currency)
     values ($1,$2,$3,$4) returning number, issued_at`,
    [orderId, number, order?.total_minor ?? 0, order?.currency ?? "GBP"],
  );
  return row ?? { number, issued_at: new Date().toISOString() };
}

export async function loadInvoiceData(orderId: string): Promise<{
  order: InvoiceOrder;
  items: InvoiceItem[];
  invoiceNumber: string;
  issuedAt: string;
} | null> {
  const order = await one<InvoiceOrder>(`select * from orders where id = $1`, [orderId]);
  if (!order) return null;
  const items = await query<InvoiceItem>(
    `select title, variant_title, sku, qty, unit_price_minor, total_minor
...
export async function renderInvoicePdf(orderId: string): Promise<Buffer | null> {
  ...
  const money = (m: number): string => formatMoney(m, base);
  ...
  for (const it of items) {
    ...
    if (it.sku) doc.fontSize(8).fillColor(grey).text(`SKU ${it.sku}`, cols.desc, y + h - 2, { width: 270 });
    y += h + (it.sku ? 10 : 4);
  }
  ...
  if (order.shipping_minor > 0) totalRow("Shipping", money(order.shipping_minor));
  if (order.tax_minor > 0) totalRow(taxLabel, money(order.tax_minor));
  ...
  if (order.currency !== base && order.fx_rate && order.fx_rate !== 1) {
    ...
    doc.font("Helvetica").fontSize(9).fillColor(grey);
    doc.text(
      `Charged as ${formatMoney(charged, order.currency)} at 1 ${base} = ${order.fx_rate.toFixed(4)} ${order.currency}`,
      330,
      y,
      { width: 217, align: "right" },
    );
  ...
  doc.fontSize(8).fillColor(grey).text(
    `${storeName} \u00b7 invoice ${invoiceNumber} \u00b7 generated ${new Date().toLocaleString("en-GB")}`,
    left,
    790,
...
export async function renderInvoiceHtml(orderId: string): Promise<string | null> {
  const data = await loadInvoiceData(orderId);
  if (!data) return null;
...
<h1>${esc(storeName)}</h1><p><strong>Invoice ${esc(invoiceNumber)}</strong> \u00b7 Order ${esc(order.number)} \u00b7 ${new Date(
    order.placed_at,
...
${items
  .map(
    (i) =>
      `<tr><td>${esc(i.title)}${i.variant_title ? ` <small>(${esc(i.variant_title)})</small>` : ""}</td><td class="r">${
        i.qty
      }</td><td class="r">${money(i.unit_price_minor)}</td><td class="r">${money(i.total_minor)}</td></tr>`,
  )
  .join("")}
</tbody></table>
