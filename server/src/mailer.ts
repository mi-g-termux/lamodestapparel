...
/**
 * SMTP is configured entirely from Admin → Settings → Email.
 * Nothing here reads .env, so the store owner can change providers themselves.
 */
...
async function getTransport(cfg?: SmtpConfig): Promise<Transporter | null> {
  const c = cfg ?? (await getSmtp());
  if (!c.host || !c.from_email) return null;
  const key = keyOf(c);
...
export async function verifySmtp(): Promise<{ ok: boolean; error?: string }> {
  ...
  if (!tx) return { ok: false, error: "SMTP host / from address is not configured" };
  try {
    await tx.verify();
    return { ok: true };
  } catch (err) {
...
async function logEmail(
  ...
  await query(
    ...
    [to.slice(0, 300), subject.slice(0, 300), template ?? null, status, error ?? null],
  ).catch(() => {});
}
...
export async function emailShell(bodyHtml: string, preheader = ""): Promise<string> {
...
export function button(label: string, href: string, colour = "#111111"): string {
