...
export const designTokensRouter: Router = Router();

// GET /api/admin/design-tokens - Get all design tokens
designTokensRouter.get("/", requireAuth, requirePerm("settings.read"), async (_req: Request, res: Response) => {
  try {
...
designTokensRouter.get("/:namespace", requireAuth, requirePerm("settings.read"), async (req: Request, res: Response) => {
  try {
...
designTokensRouter.put("/:namespace", requireAuth, requirePerm("settings.write"), async (req: Request, res: Response) => {
  try {
    const { namespace } = req.params;
    ...
    await query(
      `INSERT INTO design_tokens (namespace, tokens, updated_at)
       VALUES ($1, $2, NOW())
       ON CONFLICT (namespace) DO UPDATE SET tokens = $2, updated_at = NOW()`,
      [namespace, JSON.stringify(tokens)]
    );
...
designTokensRouter.get("/css/variables", async (_req: Request, res: Response) => {
  try {
...
designTokensRouter.get("/public/theme", async (_req: Request, res: Response) => {
  try {
    const result = await query("SELECT namespace, tokens FROM design_tokens");
    res.json(result);
  } catch (err) {
