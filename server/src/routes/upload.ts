...
function sanitiseSvg(buf: Buffer): Buffer {
...
uploadRouter.post("/", csrfGuard, upload.array("files", 10), async (req: Request, res: Response) => {
  ...
  if (isServerless) {
    res.status(400).json({
      error:
        "This deployment has a read-only filesystem (Vercel). Use \u201cAdd image by URL\u201d, " +
  ...
  const folder = String((req.body as Record<string, unknown>)?.folder ?? "misc")
  ...
  for (const f of files) {
    let data = f.buffer;
    if (f.mimetype === "image/svg+xml") data = sanitiseSvg(data);
    ...
    const existing = await one<{ id: string }>(`select id from media where url = $1`, [url]);
    const row = existing
      ? await one(`select * from media where id = $1`, [existing.id])
      : await one(
          `insert into media (url, filename, mime, bytes, alt, folder, hash)
           values ($1,$2,$3,$4,$5,$6,$7) returning *`,
          [url, f.originalname.slice(0, 200), f.mimetype, data.byteLength,
           String((req.body as Record<string, unknown>)?.alt ?? ""), folder, hash],
        );
