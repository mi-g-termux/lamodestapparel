...
stateRouter.get("/", async (req: Request, res: Response) => {
  ...
  const [media, products, variants, images, categories, orders, orderItems, history,
         customers, reviews, coupons, messages, subscribers, staff, audit, banners,
         pages, navigation, testimonials, invoices, notifications] = await Promise.all([
  ...
  const orderDocs = orders.map((o) => {
    ...
    return {
      ...
      billingAddress: (o.billing_address ?? ship) as Record<string, unknown>,
...
const asDocs = (v: unknown): Doc[] => (Array.isArray(v) ? (v as Doc[]) : []);
 ...
 */
stateRouter.put("/", async (req: Request, res: Response) => {
  const user = req.admin!;
  ...
  if (body.categories && can(user, "product.update")) {
    for (const c of asDocs(body.categories)) {
      if (isUuid(c.id)) {
        await query(
          `update categories set name = $1, slug = $2, description = $3, image_id = $4,
                  visible = $5, position = $6 where id = $7`,
          [c.name, c.slug, c.description ?? "", isUuid(c.imageId) ? c.imageId : null, c.visible !== false, Number(c.position ?? 0), c.id],
        );
      } else {
        await query(
          `insert into categories (name, slug, description, image_id, visible, position)
           values ($1,$2,$3,$4,$5,$6) on conflict (slug) do update set name = excluded.name`,
          [c.name, c.slug, c.description ?? "", isUuid(c.imageId) ? c.imageId : null, c.visible !== false, Number(c.position ?? 0)],
        );
      }
    }
  ...
  if (body.banners && can(user, "content.manage")) {
    ...
    for (const b of asDocs(body.banners)) {
      ...
      if (isUuid(b.id)) {
        await query(
          `update banners set placement=$1, title=$2, subtitle=$3, body=$4, cta_label=$5,
        ...
        const row = await one<{ id: string }>(
          `insert into banners (placement, title, subtitle, body, cta_label, cta_href, media_id,
    ...
    if (keep.length) await query(`delete from banners where id <> all($1::uuid[])`, [keep]);
    else await query(`delete from banners`);
  ...
  if (body.navigation && can(user, "content.manage")) {
    ...
    for (const n of asDocs(body.navigation)) {
      await query(
        ...
        [n.menu ?? "header", n.label ?? "", n.href ?? "/", Number(n.position ?? 0), n.visible !== false],
      );
    }
  ...
  if (body.pages && can(user, "content.manage")) {
    for (const p of asDocs(body.pages)) {
      await query(
        ...
        [p.slug, p.title, p.body ?? "", p.published !== false],
      );
    }
    applied.push("pages");
  }
  ...
  if (body.testimonials && can(user, "content.manage")) {
    ...
    for (const t of asDocs(body.testimonials)) {
      await query(
        `insert into testimonials (author, role, body, rating, avatar_url, active, position)
         values ($1,$2,$3,$4,$5,$6,$7)`,
        [t.author ?? "", t.role ?? "", t.body ?? "", Number(t.rating ?? 5), t.avatarUrl ?? null, t.active !== false, Number(t.position ?? 0)],
      );
    }
  ...
  if (body.serverSettings && can(user, "settings.write")) {
    for (const [ns, value] of Object.entries(body.serverSettings as Doc)) {
      if (value && typeof value === "object") await saveSettings(ns, value as Doc);
    }
