...
import {
  userFromRequest, createSession, destroySession, verifyPassword, hashPassword,
  passwordProblems, lockoutMinutesFor, clientIp, can, isDeveloperZone,
  csrfGuard, rateLimit, ALL_PERMISSIONS, permissionsFor,
  type AdminUser, type Permission,
} from "../security.js";
import {
  getSettingsForClient, saveSettings, auditWrite, DEFAULT_SETTINGS, invalidateSettings,
} from "../settings.js";
...
import {
  updateOrderField, revenueSeries, dashboardSummary, topProducts, previewOrderNumber,
  ORDER_STATUSES, PAYMENT_STATUSES, FULFILLMENT_STATUSES,
} from "../orders.js";
...
function requireDeveloper(req: Request, res: Response, next: NextFunction): void {
...
const audit = async (req: Request, action: string, entity: string, before?: unknown, after?: unknown): Promise<void> =>
  auditWrite(req.admin?.id ?? null, req.admin?.name ?? null, action, entity, clientIp(req), before, after);

/* ------------------------------------------------------------------ *
 * Session
 * ------------------------------------------------------------------ */

adminRouter.post("/auth/login", rateLimit("admin_login", 10, 900), async (req: Request, res: Response) => {
  ...
  }>(`select * from admin_users where lower(email) = $1`, [email]);
...
adminRouter.post("/auth/change-password", async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
  const current = String(b.currentPassword ?? "");
  const next = String(b.newPassword ?? "");
  const row = await one<{ password_hash: string }>(`select password_hash from admin_users where id = $1`, [req.admin!.id]);
...
adminRouter.get("/dashboard", requirePerm("dashboard.view"), async (req: Request, res: Response) => {
  ...
  const [summary, series, top, recent] = await Promise.all([
    ...
    query(
      `select id, number, email, status, payment_status, total_minor, currency, placed_at
...
adminRouter.get("/reports/revenue", requirePerm("report.revenue.view"), async (req: Request, res: Response) => {
  const granularity = (String(req.query.granularity ?? "month") as "day" | "week" | "month");
...
adminRouter.get("/notifications", async (req: Request, res: Response) => {
  ...
  const unread = await one<{ n: number }>(`select count(*)::int as n from notifications where read_at is null`);
  res.json({ notifications: rows, unread: unread?.n ?? 0 });
});

adminRouter.post("/notifications/read", async (req: Request, res: Response) => {
  const id = (req.body as Record<string, unknown>)?.id;
  if (id) await query(`update notifications set read_at = now() where id = $1`, [String(id)]);
  else await query(`update notifications set read_at = now() where read_at is null`);
  res.json({ ok: true });
});
...
adminRouter.get("/products", requirePerm("product.read"), async (req: Request, res: Response) => {
  ...
  const rows = await query(
    `select p.*, (select m.url from product_images pi join media m on m.id = pi.media_id
  ...
  const total = await one<{ n: number }>(
    `select count(*)::int as n from products where ($1 = '' or title ilike '%' || $1 || '%') and ($2 = '' or status = $2)`,
    [q, status],
  );
  res.json({ products: rows, total: total?.n ?? 0 });
});

adminRouter.get("/products/:id", requirePerm("product.read"), async (req: Request, res: Response) => {
...
function pickProduct(body: Record<string, unknown>): { cols: string[]; vals: unknown[] } {
  const cols: string[] = [];
  const vals: unknown[] = [];
...
adminRouter.post("/products", requirePerm("product.create"), async (req: Request, res: Response) => {
  ...
  const row = await one<{ id: string }>(
    `insert into products (slug, title, status, price_minor) values ($1,$2,$3,$4) returning id`,
    [finalSlug, title, String(body.status ?? "draft"), Number(body.price_minor ?? 0)],
  );
  await audit(req, "product.create", title, null, { id: row?.id, slug: finalSlug });
  res.status(201).json({ id: row?.id, slug: finalSlug });
});

adminRouter.patch("/products/:id", requirePerm("product.update"), async (req: Request, res: Response) => {
  const before = await one(`select * from products where id = $1`, [req.params.id]);
  if (!before) {
    res.status(404).json({ error: "Product not found" });
    return;
  }
  const { cols, vals } = pickProduct(req.body as Record<string, unknown>);
  if (cols.length) {
  ...
  if (Array.isArray(body.imageMediaIds)) {
    ...
    let pos = 0;
    for (const mid of body.imageMediaIds as string[]) {
  ...
  if (Array.isArray(body.variants)) {
    ...
    for (const v of body.variants as Record<string, unknown>[]) {
      await query(
        `insert into product_variants (product_id, title, options, sku, price_minor, stock, position)
         values ($1,$2,$3::jsonb,$4,$5,$6,$7)`,
        [req.params.id, String(v.title ?? "Default"), JSON.stringify(v.options ?? {}), v.sku ?? null,
...
adminRouter.delete("/products/:id", requirePerm("product.delete"), async (req: Request, res: Response) => {
...
adminRouter.get("/orders", requirePerm("order.read"), async (req: Request, res: Response) => {
  const limit = Math.min(200, Math.max(1, Number(req.query.limit ?? 50)));
  const offset = Math.max(0, Number(req.query.offset ?? 0));
  const q = String(req.query.q ?? "").trim();
  const status = String(req.query.status ?? "").trim();
  ...
  const rows = await query(
    `select o.*, (select count(*)::int from order_items oi where oi.order_id = o.id) as item_count,
      ...
      order by o.placed_at desc limit $4 offset $5`,
    [q, status, payment, limit, offset],
  );
  const total = await one<{ n: number }>(
    `select count(*)::int as n from orders
      where ($1 = '' or number ilike '%' || $1 || '%' or email ilike '%' || $1 || '%')
        and ($2 = '' or status = $2) and ($3 = '' or payment_status = $3)`,
    [q, status, payment],
  );
  res.json({
    orders: rows,
...
adminRouter.get("/orders/:id", requirePerm("order.read"), async (req: Request, res: Response) => {
...
adminRouter.patch("/orders/:id", requirePerm("order.update"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
  const actor = req.admin!.name;

  for (const field of ["status", "payment_status", "fulfillment_status"] as const) {
  ...
  const simple = ["tracking_number", "tracking_url", "courier", "staff_note", "shipping_method"] as const;
...
adminRouter.post("/orders/:id/refund", requirePerm("order.refund"), async (req: Request, res: Response) => {
  const amount = Math.max(0, Number((req.body as Record<string, unknown>)?.amountMinor ?? 0));
  const reason = String((req.body as Record<string, unknown>)?.reason ?? "");
  const order = await one<{ total_minor: number }>(`select total_minor from orders where id = $1`, [req.params.id]);
  if (!order) {
    res.status(404).json({ error: "Order not found" });
    return;
  }
  ...
  const refunded = await one<{ total: number }>(`select coalesce(sum(amount_minor),0)::bigint as total from refunds where order_id = $1`, [req.params.id]);
...
adminRouter.get("/orders/:id/invoice.pdf", requirePerm("invoice.download"), async (req: Request, res: Response) => {
  const pdf = await renderInvoicePdf(String(req.params.id));
  if (!pdf) {
    res.status(404).json({ error: "Order not found" });
    return;
  }
  const order = await one<{ number: string }>(`select number from orders where id = $1`, [req.params.id]);
...
adminRouter.get("/orders/:id/invoice.html", requirePerm("invoice.download"), async (req: Request, res: Response) => {
...
adminRouter.post("/orders/:id/invoice", requirePerm("invoice.download"), async (req: Request, res: Response) => {
...
adminRouter.get("/customers", requirePerm("customer.read"), async (req: Request, res: Response) => {
  const q = String(req.query.q ?? "").trim();
  const rows = await query(
    `select c.*, 
...
adminRouter.get("/customers/:id", requirePerm("customer.read"), async (req: Request, res: Response) => {
...
adminRouter.get("/banners", requirePerm("content.manage"), async (_req: Request, res: Response) => {
  res.json({
    banners: await query(
      `select b.*, m.url as media_url from banners b left join media m on m.id = b.media_id
        order by b.placement, b.position`,
    ),
  });
});

const BANNER_FIELDS = ["placement", "title", "subtitle", "body", "cta_label", "cta_href", "media_id", "image_url", "bg_color", "text_color", "position", "active", "starts_at", "ends_at"] as const;

adminRouter.post("/banners", requirePerm("content.manage"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
  const row = await one<{ id: string }>(
    `insert into banners (placement, title, subtitle, cta_label, cta_href, image_url, media_id, position, active)
     values ($1,$2,$3,$4,$5,$6,$7,$8,$9) returning id`,
    [String(b.placement ?? "hero"), b.title ?? "", b.subtitle ?? "", b.cta_label ?? "", b.cta_href ?? "",
...
adminRouter.patch("/banners/:id", requirePerm("content.manage"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
...
adminRouter.delete("/banners/:id", requirePerm("content.manage"), async (req: Request, res: Response) => {
  await query(`delete from banners where id = $1`, [req.params.id]);
  await audit(req, "banner.delete", String(req.params.id));
  res.json({ ok: true });
});

adminRouter.get("/pages", requirePerm("content.manage"), async (_req: Request, res: Response) => {
  res.json({ pages: await query(`select * from pages order by title`) });
});

adminRouter.post("/pages", requirePerm("content.manage"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
  const row = await one<{ id: string }>(
    `insert into pages (slug, title, body, published) values ($1,$2,$3,$4)
     on conflict (slug) do update set title = excluded.title, body = excluded.body,
       published = excluded.published, updated_at = now() returning id`,
    [String(b.slug ?? "").trim(), String(b.title ?? ""), String(b.body ?? ""), b.published !== false],
  );
  await audit(req, "page.save", String(b.slug ?? ""), null, b);
  res.json({ id: row?.id });
});

adminRouter.get("/navigation", requirePerm("content.manage"), async (_req: Request, res: Response) => {
  res.json({ items: await query(`select * from navigation_items order by menu, position`) });
});

adminRouter.put("/navigation", requirePerm("content.manage"), async (req: Request, res: Response) => {
  ...
  for (const it of items as Record<string, unknown>[]) {
    await query(
      `insert into navigation_items (menu, label, href, position, visible) values ($1,$2,$3,$4,$5)`,
      [String(it.menu ?? "header"), String(it.label ?? ""), String(it.href ?? "/"), Number(it.position ?? pos++), it.visible !== false],
    );
  }
...
adminRouter.get("/settings", requirePerm("settings.read"), async (req: Request, res: Response) => {
  const tree = await getSettingsForClient();
  if (!can(req.admin!, "settings.payments.read")) delete tree.payments;
  if (!can(req.admin!, "settings.smtp.read")) delete tree.smtp;
  if (!isDeveloperZone(req.admin!)) delete tree.security;
  res.json({ settings: tree, namespaces: Object.keys(DEFAULT_SETTINGS) });
});

adminRouter.put("/settings/:namespace", async (req: Request, res: Response) => {
  ...
  const needed: Permission =
    ns === "payments" ? "settings.payments.write" :
  ...
  if (ns === "smtp") resetTransport();
  if (ns === "currency") void refreshRates();
...
adminRouter.post("/settings/orders/preview", requirePerm("settings.read"), (req: Request, res: Response) => {
  res.json({ preview: previewOrderNumber(req.body as Record<string, never>) });
});

/** One-switch feature toggles. */
adminRouter.get("/features", requirePerm("settings.read"), async (_req: Request, res: Response) => {
  const tree = await getSettingsForClient();
  res.json({ features: tree.features, defaults: DEFAULT_SETTINGS.features });
});

adminRouter.post("/features/toggle", requirePerm("features.manage"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
...
adminRouter.post("/smtp/test", requirePerm("settings.smtp.write"), rateLimit("smtp_test", 5, 600), async (req: Request, res: Response) => {
  const to = String((req.body as Record<string, unknown>)?.to ?? req.admin!.email);
  ...
  const sent = await sendMail({
    ...
    html: await emailShell(
      `<h2 style="margin:0 0 8px;font-size:18px">Your email settings work</h2>
...
adminRouter.post("/currency/refresh", requirePerm("settings.write"), rateLimit("fx_refresh", 10, 600), async (req: Request, res: Response) => {
  const result = await refreshRates();
  await audit(req, "currency.refresh", "exchange_rates", null, result);
  res.json(result);
});

adminRouter.get("/currency/rates", requirePerm("settings.read"), async (_req: Request, res: Response) => {
  const tree = await getSettingsForClient();
...
adminRouter.put("/currency/rate", requirePerm("settings.write"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
  await query(
    `insert into exchange_rates (base, quote, rate, source, fetched_at) values ($1,$2,$3,'manual',now())
     on conflict (base, quote) do update set rate = excluded.rate, source = 'manual', fetched_at = now()`,
    [String(b.base), String(b.quote), Number(b.rate)],
  );
...
adminRouter.get("/media", requirePerm("media.manage"), async (req: Request, res: Response) => {
  const folder = String(req.query.folder ?? "");
  res.json({
    media: await query(
      `select * from media where ($1 = '' or folder = $1) order by created_at desc limit 300`,
      [folder],
    ),
  });
});

adminRouter.post("/media/link", requirePerm("media.manage"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
  ...
  const row = await one<{ id: string; url: string }>(
    `insert into media (url, alt, folder) values ($1,$2,$3) returning id, url`,
    [url, String(b.alt ?? ""), String(b.folder ?? "misc")],
  );
  await audit(req, "media.link", url);
  res.status(201).json(row);
});

adminRouter.delete("/media/:id", requirePerm("media.manage"), async (req: Request, res: Response) => {
...
adminRouter.get("/coupons", requirePerm("discount.manage"), async (_req: Request, res: Response) => {
  res.json({ coupons: await query(`select * from coupons order by created_at desc`) });
});

adminRouter.post("/coupons", requirePerm("discount.manage"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
  ...
  const row = await one<{ id: string }>(
    `insert into coupons (code, type, value, min_spend_minor, usage_limit, starts_at, ends_at, active)
     ...
     returning id`,
    [code, String(b.type ?? "percent"), Number(b.value ?? 0), Number(b.min_spend_minor ?? 0),
...
adminRouter.delete("/coupons/:id", requirePerm("discount.manage"), async (req: Request, res: Response) => {
  await query(`delete from coupons where id = $1`, [req.params.id]);
  res.json({ ok: true });
});

adminRouter.get("/subscribers", requirePerm("customer.read"), async (_req: Request, res: Response) => {
  res.json({ subscribers: await query(`select * from newsletter_subscribers order by created_at desc limit 500`) });
});

adminRouter.get("/messages", requirePerm("customer.read"), async (_req: Request, res: Response) => {
  res.json({ messages: await query(`select * from contact_messages order by created_at desc limit 300`) });
});

adminRouter.get("/reviews", requirePerm("content.manage"), async (_req: Request, res: Response) => {
  res.json({
    reviews: await query(
      `select r.*, p.title as product_title from product_reviews r
         join products p on p.id = r.product_id order by r.created_at desc limit 300`,
    ),
  });
});

adminRouter.patch("/reviews/:id", requirePerm("content.manage"), async (req: Request, res: Response) => {
  const state = String((req.body as Record<string, unknown>)?.state ?? "");
...
adminRouter.get("/staff", requirePerm("user.manage"), async (_req: Request, res: Response) => {
  res.json({
    staff: await query(
      `select id, email, name, role, status, overrides, last_login_at, failed_attempts, locked_until, created_at
...
adminRouter.post("/staff", requirePerm("user.manage"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
  ...
  const row = await one<{ id: string }>(
    ...
    [email, String(b.name ?? email), hashPassword(password), String(b.role ?? "staff")],
  );
  await audit(req, "staff.create", email, null, { role: b.role });
  res.status(201).json({ id: row?.id });
});

adminRouter.patch("/staff/:id", requirePerm("user.manage"), async (req: Request, res: Response) => {
  const b = req.body as Record<string, unknown>;
  if (req.params.id === req.admin!.id && b.status === "suspended") {
    res.status(400).json({ error: "You cannot suspend your own account." });
    return;
  }
  for (const f of ["name", "role", "status"] as const) {
    if (b[f] !== undefined) await query(`update admin_users set "${f}" = $1 where id = $2`, [b[f], req.params.id]);
  }
...
adminRouter.get("/audit", requirePerm("audit.view"), async (req: Request, res: Response) => {
...
adminRouter.get("/system", requireDeveloper, async (_req: Request, res: Response) => {
...
adminRouter.post("/system/cache/clear", requireDeveloper, async (req: Request, res: Response) => {
  invalidateSettings();
  resetTransport();
...
adminRouter.post("/system/sessions/revoke-all", requireDeveloper, async (req: Request, res: Response) => {
